...
class FinancialAgent:
    ...
    # ── The decision loop ───────────────────────────────────────
    def run(self, input_data: SalaryInput) -> SavingsPlan:
        """
    ...
    def _plan(self, data: SalaryInput, analysis: dict) -> SavingsPlan:
        """
        PLANNING: build the final savings plan using
        the analysis results and generate recommendations.
        """
        from agent.rules import build_plan
        self._log("building savings plan")
        plan = build_plan(data, analysis)
        self._log(f"plan score: {plan.plan_score}/100")
        self._log(f"current savings: ₹{plan.current_savings:,.0f}/month")
        self._log(f"recommended savings: ₹{plan.recommended_savings:,.0f}/month")
        self.plan = plan
        return plan
